...
import {
  ...
  searchAllEvaluationService, searchByCompanyIdEvaluationService,
  updateEvaluationService
} from "@/api/evaluation.js";
import {ref} from "vue";
import useEvaluationInfoStore from "@/stores/evaluationInfo.js";
...
const getInfo = async()=>{
  ...
  try {
    // 调用接口并获取结果
    let result = await searchByCompanyIdEvaluationService(userInfo.value.companyId);
    // 打印结果
...
<!--      </el-form-item>-->
      ...
      <el-form-item label="用人单位分数" >
        <el-input v-model="addModel.companyScore" placeholder="请输入内容" disabled="disabled"></el-input>
      </el-form-item>
...
<!--      <el-form-item label="用人单位分数" >-->
<!--        <el-input v-model="updateModel.companyScore" placeholder="请输入标题" disabled="disabled"></el-input>-->
<!--      </el-form-item>-->
      <el-form-item label="评价内容" >
