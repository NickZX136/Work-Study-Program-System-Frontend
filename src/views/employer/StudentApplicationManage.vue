...
import {
  ...
  searchStudentApplicationByCompanyIdService, updateStudentApplicationService
...
const getInfo = async()=>{
  ...
  try {
    // 调用接口并获取结果
    let result = await searchStudentApplicationByCompanyIdService(userInfo.value.companyId);
    // 打印结果
...
<template>
  ...
  <el-drawer v-model="updateVisibleDrawer" title="修改留言" direction="rtl" size="50%">
    ...
    <el-form :model="updateModel" label-width="100px" >
      ...
      <el-form-item label="反馈结果" >
        <el-input v-model="updateModel.feedbackResult" placeholder="请输入标题"></el-input>
      </el-form-item>
      <el-form-item label="反馈时间" >
        <el-input v-model="updateModel.feedbackTime" placeholder="请输入标题"></el-input>
      </el-form-item>
  ...
  <el-drawer v-model="visibleDrawer" title="添加应聘信息" direction="rtl" size="50%">
    ...
    <el-form :model="addModel" label-width="100px" >
      ...
      <el-form-item label="反馈结果" >
        <el-input type="textarea" v-model="addModel.feedbackResult" placeholder="请输入内容"></el-input>
      </el-form-item>
      <el-form-item label="反馈时间" >
        <el-input v-model="addModel.feedbackTime" placeholder="请输入内容"></el-input>
      </el-form-item>
      <el-form-item label="过期状态" >
