...
console.log("userInfo.value.companyId",userInfo.value.studentId)
...
//TODO:编程式导航至留言详情页面，并传递消息ID还没改
// 编程式导航至留言详情页面，并传递消息ID
const showMessageDetail = (row) => {
  // 编程式导航到消息详情页面，并将消息 ID 作为参数传递
  router.push({ path: '/stu/StuMessageManagementDetail', query: { messageId: row.messageId } });
};
